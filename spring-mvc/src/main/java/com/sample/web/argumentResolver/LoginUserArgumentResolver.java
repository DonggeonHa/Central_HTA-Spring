...
/*
    HandlerMethodArgumentResolver
        - 요청핸들러 메소드의 매개변수를 분석해서 적절한 값을 객체의 해당 매개변수에 제공하는 객체들이 구현하는 인터페이스
        - 주요 메소드
            ...
            Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory)
                ...
                - 이 메소드의 실행결과로 반홚되는 객체가 해당 매개변수에 전달된다.
 */
...
public class LoginUserArgumentResolver implements HandlerMethodArgumentResolver  {
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        ...
        //      해당 파라미어테 @LoginUser 어노테이션이 지정되어 있는 @LoginUser 객체가 반환되고, 지정되어 있지 않으면 null이 반환된다.
        //  Objects.nonNull(객체)
        //      rorcprk null이 아니면 true를 반환하고 null이면 false를 바노한한다.
        return Objects.nonNull(parameter.getParameterAnnotation(LoginUser.class));
    }

    // 위에 정의된 supportsParameter() 메소드가 true를 반환할 떄만 이 메소드가 실행된다.
    // 매개변수에 @LoginUser 어노테이션이 지정되어 있는 경우에만 이 메소드가 실행된다.
    // HttpSession객체에서 LOGINED_USER 라는 속성명으로 저장된 객체를 반환하도록 재정의하였다.
    @Override
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        // SessionUtils.getAttribute() 메소드를 사용해서 HttpSession 객체에서 지정된 속성명으로 조회되는 객체를 반환함.
        // 사용자 인증이 완료된 사용자의 요청인 경우 User객체(로그인된 사용자정보가 포함된)가 반환되고, 사용자 인증이 완료됮 않은 경우 null 이 반환됨
